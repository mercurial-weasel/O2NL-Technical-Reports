import { StaffMember, StaffData, monthFormatter } from './types';
...
/**
 * Calculate staff movement (onboarding/offboarding) across different categories
 */
export function calculateStaffMovement(data: StaffData): StaffMovement {
  ...
  data.forEach(staff => {
    ...
    const endMonth = endDate.toISOString().slice(0, 7); // YYYY-MM format
