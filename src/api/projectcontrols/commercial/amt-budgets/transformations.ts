...
export function calculateBudgetMetricsAMT(data: AMTBudgetData[string]): BudgetMetric[] {
  return [
    {
      title: 'Total Budget',
...
export function getAvailableMonthsAMT(data: AMTBudgetData): string[] {
  return Object.keys(data).sort((a, b) => b.localeCompare(a)); // Sort in descending order
}

export function getMonthlyDataAMT(data: AMTBudgetData, month: string) {
  return data[month];
}