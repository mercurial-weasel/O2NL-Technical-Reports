...
export function AMTDashboard() {
  ...
  const [amtData, setAMTData] = useState<AMTMonthlyRecord[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const error = err instanceof Error ? err : new Error('Failed to load AMT data');
        logger.error('AMT data fetch failed', { error: error.message });
        setError(error);
      } finally {
        setIsLoading(false);
      }
  ...
  const availableMonths = React.useMemo(() => 
    Array.from(new Set(amtData.map(record => record.month))).sort((a, b) => b.localeCompare(a)),
  [amtData]);

  // Get current month's data
  const currentMonthData = React.useMemo(() => 
    amtData.filter(record => record.month === selectedMonth),
  [amtData, selectedMonth]);
        ...
        <div className="pt-24">
          <Section className="py-8">
            <div className="flex items-center justify-center h-64">
              <div className="text-text-secondary">Loading AMT data...</div>
            </div>
          </Section>
        </div>
        <Footer />
          ...
          <Section className="py-8">
            <div className="bg-red-500/10 border border-red-500 rounded-lg p-4">
              <div className="text-red-400">
                Error loading AMT data: {error.message}
              </div>
            </div>
          </Section>
        </div>
