...
import { GeoCBR, formatCBRData } from '@api/geotechnical/cbr';
    ...
    >
      <Popup>
        <div className="min-w-[200px]">
          <h3 className="font-bold">{item.sample_id}</h3>
          <p><span className="font-medium">Investigation:</span> {item.investigation_id}</p>
          <p><span className="font-medium">Depth:</span> {item.depth_to.toFixed(1)}m</p>
          <p><span className="font-medium">CBR (%):</span> {item.cbr_perc !== null ? `${item.cbr_perc.toFixed(1)}` : 'N/A'}</p>
          <p><span className="font-medium">Water Content (%):</span> {item.water_content_compacted !== null ? `${item.water_content_compacted.toFixed(1)}` : 'N/A'}</p>
          <p><span className="font-medium">Dry Density (t/m³):</span> {item.dry_density !== null ? `${item.dry_density.toFixed(3)}` : 'N/A'}</p>
          {item.chainage && (
            <p className="text-xs mt-2 text-gray-500">Chainage: {item.chainage.toFixed(1)}</p>
          )}
          {item.latLng && (
            <p className="text-xs text-gray-500">Coordinates (WGS84): [{item.latLng[0].toFixed(6)}, {item.latLng[1].toFixed(6)}]</p>
          )}
          {item.geological_unit && (
            <p className="text-xs text-gray-500">Geological Unit: {item.geological_unit}</p>
          )}
        </div>
      </Popup>
    </Marker>
...
export const CBRMap: React.FC<CBRMapProps> = ({ 
  data, 
  selectedItems, 
  onItemSelect,
  onPopupOpen
  ...
  const formattedData = useMemo(() => {
    return data.map(item => formatCBRData(item));
  }, [data]);
  ...
  useEffect(() => {
    const styleId = 'cbr-map-marker-styles';
