...
import { GeoMDD, formatMDDData } from '@api/geotechnical/mdd';
    ...
    >
      <Popup>
        <div className="min-w-[200px]">
          ...
          <p><span className="font-medium">Depth:</span> {item.SAMPLE_TOP.toFixed(1)}m</p>
          <p><span className="font-medium">Sample Type:</span> {item.SAMPLE_TYPE}</p>
          <p><span className="font-medium">NMC:</span> {item.NMC !== null ? `${item.NMC}%` : 'N/A'}</p>
          <p><span className="font-medium">Max Dry Density:</span> {item.DryDensity !== null ? `${item.DryDensity} t/m³` : 'N/A'}</p>
          <p className="text-xs mt-2 text-gray-500">
            {item.x && item.y 
              ? `Coordinates (NZTM): [${Number(item.x).toFixed(1)}, ${Number(item.y).toFixed(1)}]`
...
const MDDMap: React.FC<MDDMapProps> = ({ 
  data, 
  selectedItems, 
  onItemSelect,
  onPopupOpen
  ...
  const formattedData = useMemo(() => {
    return data.map(item => formatMDDData(item));
  }, [data]);
  ...
  useEffect(() => {
    const styleId = 'mdd-map-marker-styles';
